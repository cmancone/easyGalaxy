...
def build_grid_astro_filter():
	...
	astro_filter = ezgal.astro_filter( tests.v_filter, cosmology=Cosmology(), vega=tests.vega, solar=tests.solar )
	astro_filter.grid( tests.zf, tests.bc03_vs, tests.grid_zs, tests.bc03_ages, tests.bc03 )
...
class test_gridded_funcs(unittest.TestCase):
	...
	def setUpClass( cls ):
		...
		cls.astro_filter.grid_solar( tests.zf, tests.solar[:,0], tests.solar[:,1] )
		
		# and the masses
		cls.astro_filter.grid_masses( tests.zf, tests.bc03_ages, tests.bc03_masses )
	...
	def test_apparent_mags( self ):
		...
		self.assertTrue( np.allclose(
			...
			np.asarray([ 37.08456776,  42.39315957,  44.61016611,  47.43454964,  48.24942464]),
			1e-6
		) )
	...
	def test_apparent_mags_vega( self ):
		...
		self.assertTrue( np.allclose(
			self.astro_filter.get_apparent_mags( tests.zf, tests.test_zs, vega=True ),
			np.asarray([ 37.09150655,  42.40009836,  44.6171049 ,  47.44148843,  48.25636343]),
			1e-6
		) )
	...
	def test_apparent_mags_invalid_zf( self ):
		...
		with self.assertRaises( ValueError ):
			self.astro_filter.get_apparent_mags( tests.zf+1, tests.test_zs, vega=True )
	...
	def test_absolute_mags( self ):
		...
		self.assertTrue( np.allclose(
			...
			np.asarray([ 1.40721352,  1.13022703,  1.12120338,  2.50513901,  2.55585118]),
			1e-6
		) )
	...
	def test_absolute_mags_vega( self ):
		...
		# make sure it remembers to calculate vega magnitudes
		self.assertTrue( np.allclose(
			self.astro_filter.get_absolute_mags( tests.zf, tests.test_zs, vega=True ),
			np.asarray([ 1.41415231,  1.13716582,  1.12814217,  2.5120778 ,  2.56278996]),
			1e-6
		) )
	...
	def test_absolute_mags_invalid_zf( self ):
		...
		# it should raise an exception if we request a non-gridd zf
		with self.assertRaises( ValueError ):
			self.astro_filter.get_absolute_mags( tests.zf+1, tests.test_zs, vega=True )
	...
	def test_observed_absolute_mags( self ):
		...
		# standard calling sequence
		self.assertTrue( np.allclose(
			self.astro_filter.get_observed_absolute_mags( tests.zf, tests.test_zs ),
			np.asarray([ 1.32404124,  0.04532745, -0.09802977,  1.41568863,  1.48096523]),
			1e-6
		) )
	...
	def test_observed_absolute_mags_vega( self ):
		...
		# make sure it remembers to calculate vega magnitudes
		self.assertTrue( np.allclose(
			self.astro_filter.get_observed_absolute_mags( tests.zf, tests.test_zs, vega=True ),
			np.asarray([ 1.33098003,  0.05226624, -0.09109098,  1.42262742,  1.48790402]),
			1e-6
		) )
	...
	def test_observed_absolute_mags_invalid_zf( self ):
		...
		# it should raise an exception if we request a non-gridd zf
		with self.assertRaises( ValueError ):
			self.astro_filter.get_observed_absolute_mags( tests.zf+1, tests.test_zs, vega=True )
	...
	def test_kcorrects( self ):
		...
		self.assertTrue( np.allclose(
			...
			np.asarray([-0.08317228, -1.08489958, -1.21923315, -1.08945039, -1.07488595]),
			1e-6
		) )
	...
	def test_ekcorrects( self ):
		...
		self.assertTrue( np.allclose(
			...
			np.asarray([-0.08317228, -1.36188607, -1.50524329,  0.00847511,  0.07375171]),
			1e-6
		) )
	...
	def test_solar_mags( self ):
		...
		self.assertTrue( np.allclose(
			...
			np.asarray([  4.84424744,   5.89246026,   9.15878231,  13.14447493,  16.62662314]),
			1e-6
		) )
	...
	def test_solar_mags_vega( self ):
		...
		# make sure it remembers to calculate vega magnitudes
		self.assertTrue( np.allclose(
			self.astro_filter.get_solar_mags( tests.zf, tests.test_zs, vega=True ),
			np.asarray([  4.85118623,   5.89939905,   9.1657211 ,  13.15141372,  16.63356193]),
			1e-6
		) )
	...
	def test_solar_mags_invalid_zf( self ):
		...
		with self.assertRaises( ValueError ):
			self.astro_filter.get_solar_mags( tests.zf+1, tests.test_zs, vega=True )
	...
	def test_masses( self ):
		...
		# standard calling sequence
		self.assertTrue( np.allclose(
			self.astro_filter.get_masses( tests.zf, tests.test_zs ),
			np.asarray([ 1.       ,  1.       ,  0.941732 ,  0.8794115,  0.872226 ]),
			1e-6
		) )
