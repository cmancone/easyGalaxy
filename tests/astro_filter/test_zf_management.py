...
class test_zf_management(unittest.TestCase):
	...
	def test_get_zf_grid( self ):
		...
		self.assertEqual( type( self.astro_filter.get_zf_grid( tests.zf ) ), ezgal.zf_grid.zf_grid )
	...
	def test_extend_zf_list_empty( self ):
		...
		self.assertTrue( np.array_equal( self.astro_filter.extend_zf_list( [] ), [3] ) )
	...
	def test_extend_zf_list_tolerance_good( self ):
		...
		# only one entry
		zfs = self.astro_filter.extend_zf_list( [3+self.astro_filter.tol*0.9] )
	...
	def test_extend_zf_list_tolerance_bad( self ):
		...
		zfs = self.astro_filter.extend_zf_list( [3+self.astro_filter.tol*1.1] )
