...
class csp_integrator(object):
    ...
    def generate_regions(self, resampling=1, ls=None):
        ...
        if resampling == 1:
            ...
            if self.has_masses:
                masses = np.interp(ages, self.ages, self.masses)
            ...
            if limit:
                for (i, ind) in enumerate(inds):
                    seds[i, :] = np.interp(ages, self.ages, self.seds[ind, :])
            else:
                for (i, age) in enumerate(ages):
                    seds[:, i] = self.interp_sed(age)

        # generate a list of region integrator objects and store
        self.regions = []
        for i in range(self.breaks.size - 1):
            region = csp_region_integrator(seds, ls, ages, self.breaks[i],
                                           self.breaks[i + 1])
    ...
    def resample_ages(self, factor):
        return np.interp(
            np.arange(0, self.ages.size - 1 + 1.0 / factor, 1.0 / factor),
            np.arange(self.ages.size), self.ages)
    ...
    def interp_sed(self, age):
        ...
        # now interpolate
        return self.seds[:, yind] + (self.seds[:, oind] - self.seds[:, yind]
                                     ) * (age - yage) / (oage - yage)
    ...
    def integrate(self, resampling=1, ls=None, return_all=True):
        ...
        if not self.has_sfh:
            raise ValueError(
                'Set a star formation history before integrating!')
        ...
        if norm <= 0:
            raise ValueError(
                'There is no star formation in the star formation history!')
    ...
    def find_resampling(self, max_err, max_iter=200):
        ...
        while True:
            ...
            # and integrate
            this = self.integrate(resampling=resampling,
                                  ls=ls,
                                  return_all=False)
            ...
            err = np.max(np.abs(-2.5 * np.log10((this_compare[m] /
                                                 last_compare[m]))))
        ...
        if niter > max_iter:
            print(
                "WARNING: Reached iteration limit of %d iterations.  Expect errors in final magnitudes of %.4f mags.\nIf this is unacceptable try increasing max_iter."
                % (max_iter, err))
...
class csp_region_integrator(object):
    ...
    def get_sfh_normalization(self):
        if not self.has_sfh:
            raise ValueError(
                'You must set the SFH function before getting a normalization!')
    ...
    def integrate(self, age):
        ...
        inds = np.where((self.ages >= lower_bound - age * 1e-5) & (
            self.ages <= upper_bound + age * 1e-5))[0]
        ...
        if inds[0] > 0 and np.abs(self.ages[inds[0]] -
                                  lower_bound) > 1e-5 * age:
        ...
        if self.has_dust:
            ...
            seds = integrate.simps(weights * self.seds[:, inds] *
                                   self.dust_func(self.ages[inds], self.ls),
                                   x=self.ages[inds],
                                   even='avg')
        else:
        ...
        mass = integrate.simps(weights * self.masses[inds],
                               x=self.ages[inds],
                               even='avg') if self.has_masses else 0
